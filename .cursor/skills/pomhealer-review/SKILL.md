---
name: pomhealer-review
description: Human-in-the-loop review of pomhealer-queue patches (patch_ready). Interactive terminal menu or agent-guided heal/skip/defer with review cards. Use when the user runs /pomhealer-review.
...
`pomhealer-review` auto-merges leftover `gh run download` folders (e.g. `pomhealer-pipeline-e2e/`) into `pomhealer-artifacts/` on startup, then **archives applied/skipped patch files** so they do not sit in pending `patches/` (including `-agent-task.md` and CI re-imports). After CI:

```bash
gh run download
python -m pomhealer.review --interactive
# or simply (TTY default):
python -m pomhealer.review
...
1. Run `python -m pomhealer.review --promote-all` if any patches may still be `awaiting_agent`. (Review already auto-imported CI downloads; `pomhealer-import` is optional if you downloaded artifacts without opening review yet.)
2. Run `python -m pomhealer.review --list` (read-only table of `patch_ready` patches).
3. For **each** pending patch (one at a time — wait for decision before the next):
   - Run `python -m pomhealer.review --show P-<id>` for the review card.
   ...
   - Present the card in this **exact fixed order every time** (do not omit sections; use `(none)` when empty):
     ...
     - **Show failure report** — read `pomhealer-artifacts/failures/F-*.md` (+ screenshot) and re-ask
...
`--yes` confirms high-risk patches when the user already approved in chat. Runs validation, updates `pages/*.py` via `pomhealer.pom_apply`, then **moves all related pending files** (`P-<id>.json`, `.md`, `-agent-task.md`) to `pomhealer-artifacts/pomhealer-queue/applied/`. Pending `patches/` must not keep leftovers. Report success/failure; then continue to the next patch.
...
```bash
python -m pomhealer.review --patch P-<id> --decision skip --reason "suspected app regression" --yes
```

If the user did not give a reason, ask once for a short reason, then run skip. Writes `pomhealer-artifacts/pomhealer-queue/skipped/P-<id>-rca.md` and moves `P-<id>.json` / `.md` / `-agent-task.md` out of pending `patches/` into `skipped/`.

### Defer (after user chose Defer)

```bash
python -m pomhealer.review --patch P-<id> --decision defer --yes
```
...
```bash
python -m pomhealer.review --patch P-<id> --decision heal --dry-run
```
