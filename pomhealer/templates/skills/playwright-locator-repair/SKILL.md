...
description: Repair broken Playwright locators in Page Object files from pytest failures, pomhealer-artifacts/failures/F-*.json reports, Playwright MCP snapshots, and pomhealer-queue patches. Use when diagnosing failing POM tests, proposing fixes in pages/*.py, or writing P-*.json patch proposals with architecture_updates.
...
Repair broken Playwright locators in `pages/*.py` safely and minimally. Use with `/pomhealer-propose` (MCP + proposals). Applying patches is `/pomhealer-review` + `pomhealer.pom_apply`.

## Workflow

1. Read `pomhealer-artifacts/failures/F-<id>.json` and `.md`, plus `pomhealer-artifacts/architecture/manifest.json`.
...
5. Write `pomhealer-artifacts/pomhealer-queue/patches/P-<id>.json` with `architecture_updates` targeting **one property** in `pages/*.py`.
