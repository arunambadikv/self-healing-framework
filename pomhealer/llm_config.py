...
def normalize_provider(raw: str | None) -> str:
    ...
    if provider not in PROVIDERS:
        raise LlmConfigError(
            f"Unknown POMHEALER_LLM_PROVIDER={raw!r}; expected one of: "
...
def resolve_llm_config(*, require_key: bool = False) -> LlmConfig:
    ...
    provider = normalize_provider(os.environ.get("POMHEALER_LLM_PROVIDER"))
    key_env = KEY_ENV_VARS[provider]
    api_key = _read_api_key(provider, key_env)

    model = (
        os.environ.get("POMHEALER_LLM_MODEL", "").strip()
        or (os.environ.get("POMHEALER_MCP_MODEL", "").strip() if provider == "cursor" else "")
    ...
    if require_key and not cfg.has_key:
        raise LlmConfigError(
            f"{key_env} required for POMHEALER_LLM_PROVIDER={provider}. "
...
def missing_key_message(cfg: LlmConfig) -> str:
    ...
    return (
        f"{cfg.key_env} required for MCP propose (POMHEALER_LLM_PROVIDER={cfg.provider}).\n"
        "  Capture/scan/stub/review/apply work without it.\n"
        f"  Set key:  export {cfg.key_env}=...  (or put it in .env)\n"
        f"{extra}"
        "  Check:    pomhealer-doctor"
    )
