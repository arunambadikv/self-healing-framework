...
from pomhealer.locator_source import read_property_return_expression, resolve_page_path
from pomhealer.paths import FAILURES_DIR, MANIFEST_JSON, QUEUE_PATCHES, ensure_queue_dirs
...
3. Update the existing stub at `pomhealer-artifacts/pomhealer-queue/patches/{patch_id}.json` (patch id `{patch_id}`)
...
> Complete this patch via `/pomhealer-propose` with Playwright MCP, then review with `/pomhealer-review`.
"""
...
def process_failure_entry(entry: dict[str, Any], *, workspace: Path) -> str | None:
    ...
    from pomhealer.session_state import note_session_patch
