...
from pomhealer.test_policy import check_test_policy
...
def aggregate_pomhealer_reports(
    reports_dir: Path, ignore_globs: list[str] | None = None
) -> PomhealerAggregate:
    aggregate = PomhealerAggregate()
...
def _healed_ratio(aggregate: PomhealerAggregate) -> float:
    if aggregate.total_events == 0:
        return 0.0
    return aggregate.healed / aggregate.total_events


def _repeat_healed_keys(aggregate: PomhealerAggregate, threshold: int) -> list[str]:
    counts: dict[str, int] = {}
...
def run_ci_gates(
    *,
    ...
    if config.require_architecture_manifest:
        ...
        if not MANIFEST_JSON.exists():
            errors.append(
                ...
                "Run: python -m pomhealer.architecture_scan"
            )
        else:
            ...
            if old.get("content_hash") != current.get("content_hash"):
                warnings.append(
                    "Architecture manifest is stale. Run: python -m pomhealer.architecture_scan"
                )

    if not skip_queue_gates:
        from pomhealer.queue import list_awaiting_agent, list_patch_ready, list_unprocessed_failures
        ...
        if unprocessed > config.max_unprocessed_failures:
            errors.append(
                ...
                f"({config.max_unprocessed_failures}). Run pomhealer-propose or review."
            )
        ...
        if patch_ready > config.max_patch_ready_without_review:
            warnings.append(
                ...
                f"({config.max_patch_ready_without_review}). Run /pomhealer-review."
            )
    ...
    aggregate = aggregate_pomhealer_reports(reports_dir, config.ignore_report_globs)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run CI gates: test policy, architecture manifest, pomhealer thresholds."
    )
    parser.add_argument("--workspace", default=".", help="Workspace root.")
    parser.add_argument(
        "--config",
        default="pomhealer/ci_gates_config.yaml",
        help="Gate config YAML path.",
    )
    parser.add_argument(
        "--reports-dir",
        default="pomhealer-artifacts/pomhealer-reports",
        help="Pomhealer reports directory.",
    )
    parser.add_argument("--tests-dir", default="tests", help="Tests directory.")
    parser.add_argument(
        "--skip-reports",
        action="store_true",
        help="Skip pomhealer report threshold checks (policy + queue only).",
    )
    parser.add_argument(
        "--skip-queue-gates",
        action="store_true",
        help="Skip pomhealer-queue checks (unprocessed/awaiting_agent/patch_ready).",
    )
    ...
    if config_path.exists():
        ...
        config = gate_config_from_raw(load_pomhealer_yaml(workspace))
        print(f"[pomhealer] Using packaged gate config (no {config_path})")

    reports_dir = workspace / args.reports_dir

    if args.skip_reports:
        reports_dir = workspace / "pomhealer-artifacts" / "pomhealer-reports-nonexistent-skip"
    ...
    if not args.skip_reports:
        aggregate = aggregate_pomhealer_reports(
