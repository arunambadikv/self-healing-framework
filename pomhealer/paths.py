"""Artifact directory layout for POM pomhealer pipeline (workspace-aware)."""
...
class DynamicPath:
    ...
    def resolve_path(self) -> Path:
        from pomhealer.config import get_config
...
def get_workspace() -> Path:
    ...
    env = os.environ.get("POMHEALER_WORKSPACE")
    if env:
...
def configure_workspace(root: Path | None = None) -> Path:
    ...
    from pomhealer.config import reset_config_cache
    ...
    os.environ["POMHEALER_WORKSPACE"] = str(_workspace_override)
    return _workspace_override
...
def reset_workspace() -> None:
    ...
    reset_config_cache()
    os.environ.pop("POMHEALER_WORKSPACE", None)
...
QUEUE_DIR = DynamicPath("artifacts", "pomhealer-queue")
QUEUE_INDEX = DynamicPath("artifacts", "pomhealer-queue", "index.json")
QUEUE_PATCHES = DynamicPath("artifacts", "pomhealer-queue", "patches")
QUEUE_APPLIED = DynamicPath("artifacts", "pomhealer-queue", "applied")
QUEUE_SKIPPED = DynamicPath("artifacts", "pomhealer-queue", "skipped")
QUEUE_SUMMARIES = DynamicPath("artifacts", "pomhealer-queue", "summaries")
REPORTS_DIR = DynamicPath("artifacts", "pomhealer-reports")
