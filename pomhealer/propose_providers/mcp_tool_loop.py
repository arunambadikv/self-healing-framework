...
from pomhealer.mcp_stdio import McpStdioConfig, load_playwright_mcp_stdio
...
READ_FILE_PREFIXES = ("pomhealer-artifacts/", "pages/", "tests/")
WRITE_FILE_PREFIXES = ("pomhealer-artifacts/pomhealer-queue/patches/",)
...
WORKSPACE_FILE_TOOLS: list[dict[str, Any]] = [
    {
        ...
        "function": {
            ...
            "description": (
                "Read a UTF-8 file under pages/, tests/, or pomhealer-artifacts/. "
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Write UTF-8 content to pomhealer-artifacts/pomhealer-queue/patches/ only. "
            ...
            "parameters": {
                ...
                "properties": {
                    "path": {
                        "type": "string",
                        "description": "Workspace-relative path under pomhealer-queue/patches/",
                    },
    ...
    for attempt in range(_RATE_LIMIT_RETRIES):
        try:
            ...
            if getattr(exc, "status_code", None) == 413 or "request too large" in str(exc).lower():
                ...
                print("[pomhealer] Request too large; compacted history...")
            delay = retry_delay_seconds(exc, fallback=25 + attempt * 10)
            print(f"[pomhealer] Rate limited; retrying in {delay:.0f}s...")
            time.sleep(delay)
    ...
    async with playwright_mcp_session(workspace, storage_state=storage_state) as (session, tools):
        ...
        messages: list[dict[str, Any]] = [
            {
                "role": "system",
                "content": (
                    "You are a Playwright POM pomhealer agent. "
                    ...
                    "pomhealer-artifacts/pomhealer-queue/patches/*.json and *.md. "
        ...
        for _ in range(max_steps):
            try:
                ...
                print("[pomhealer] Model called an unknown tool; retrying with a correction...")
