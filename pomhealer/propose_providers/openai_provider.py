...
from pomhealer.propose_providers.mcp_tool_loop import openai_tool_loop, run_async
    ...
    def run_propose(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(
                "openai package missing — reinstall pomhealer (openai ships with the package)"
            ) from exc
        try:
            import mcp  # noqa: F401
        except ImportError as exc:
            raise RuntimeError(
                "mcp package missing — reinstall pomhealer (mcp ships with the package)"
            ) from exc
