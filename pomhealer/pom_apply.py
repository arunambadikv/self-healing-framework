"""Apply pomhealer-queue POM patches to pages/*.py."""
...
def is_validation_command_allowed(argv: list[str]) -> bool:
    ...
    if first.startswith("python"):
        if len(argv) >= 3 and argv[1] == "-m" and argv[2] == "pytest":
            return True
        if len(argv) >= 3 and argv[1] == "-m" and argv[2].startswith("pomhealer."):
            return True
        return False
    return False
...
def parse_validation_command(validation_cmd: str) -> list[str]:
    ...
    if not is_validation_command_allowed(argv):
        raise ValueError(
            ...
            "(pytest, python -m pytest, or python -m pomhealer.*); "
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Apply POM pomhealer patch to pages/*.py")
