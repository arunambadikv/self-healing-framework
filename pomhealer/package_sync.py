...
def pin_playwright_mcp_config(workspace: Path) -> bool:
    ...
    if not path.is_file():
        from pomhealer.init import _write_mcp_stub
...
def refresh_packaged_assets(workspace: Path, *, force: bool = False) -> dict[str, Any]:
    ...
    Does not overwrite pomhealer-artifacts/pomhealer.toml or consumer .env.
    Bundled Cursor skills are owned by the package.
    """
    from pomhealer.init import _copy_skill_templates, _write_env_example
...
def refresh_packaged_assets_if_stale(workspace: Path, *, quiet: bool = False) -> dict[str, Any]:
    ...
    if result.get("refreshed") and not quiet:
        ...
        print("[pomhealer] refreshed packaged assets after install/update: " + ", ".join(parts))
        print("[pomhealer] merge any new keys from .env.example into .env (do not commit .env)")
    return result
