"""Pytest plugin hooks for POM pomhealer automation (capture + session chain)."""
...
from pomhealer.step_trace import StepTraceCollector, reset_step_trace, set_active_step_trace


def pytest_addoption(parser) -> None:
    group = parser.getgroup("pomhealer", "Playwright pomhealer framework")
    group.addoption(
        "--pomhealer-scan-architecture",
        action="store_true",
        default=False,
    ...
    group.addoption(
        ...
        action="store_true",
        default=False,
        help="Run tests marked pomhealer_demo (opens browser; set POMHEALER_DEMO_HEADLESS=1 for CI).",
    )
...
def pytest_configure(config) -> None:
    ...
    from pomhealer.playwright_trace import install_playwright_tracing
...
def pytest_sessionstart(session) -> None:
    from pomhealer.auto_scan import run_auto_architecture_discovery
    from pomhealer.paths import configure_workspace
    from pomhealer.playwright_trace import install_playwright_tracing
    from pomhealer.session_state import reset_session_state
    ...
    force = bool(session.config.getoption("--pomhealer-scan-architecture"))
    run_auto_architecture_discovery(workspace, force=force)


def pytest_sessionfinish(session, exitstatus) -> None:
    workspace = Path(session.config.rootpath)
    from pomhealer.post_test import run_post_test_chain
    from pomhealer.playwright_trace import uninstall_playwright_tracing
...
def pytest_collection_modifyitems(config, items) -> None:
    if not config.getoption("--run-demo-session"):
        skip_demo = pytest.mark.skip(
            reason="Deprecated: use pytest --run-pomhealer-demo tests/test_orangehrm_pomhealer.py"
        )
        for item in items:
            if "demo_session" in item.keywords:
                item.add_marker(skip_demo)

    if not config.getoption("--run-pomhealer-demo"):
        skip_healing = pytest.mark.skip(
            reason="Healing demo only: pytest --run-pomhealer-demo tests/test_orangehrm_pomhealer.py"
        )
        for item in items:
            if "pomhealer_demo" in item.keywords:
...
def pytest_runtest_makereport(item, call):
    """Capture healable failures into pomhealer-artifacts/failures."""
    outcome = yield
    ...
    base_url = os.environ.get("POMHEALER_BASE_URL", "")
    page_url = None
    page = None
    try:
    ...
    try:
        from pomhealer.failure_report import capture_from_exception, new_failure_id
        ...
        print(
            f"\n[pomhealer] failure captured: {failure_id}\n"
            f"  json={json_path}\n  md={md_path}\n"
            f"  next: python -m pomhealer.pom_propose --failure-id {failure_id}\n"
            f"  or: set POMHEALER_MCP_AUTO=1 in .env (or export) to auto-run scan → stub → MCP at session end"
        )
    except Exception as hook_exc:
        print(f"\n[pomhealer] failure capture error: {hook_exc}")
