...
from pomhealer.paths import MANIFEST_JSON


def _load_pomhealer_config(workspace: Path) -> dict[str, Any]:
    from pomhealer.gates_config import load_pomhealer_yaml

    return load_pomhealer_yaml(workspace)
...
def is_auto_enabled(workspace: Path | None = None) -> bool:
    ...
    if os.environ.get("POMHEALER_MCP_AUTO", "").strip() in ("1", "true", "yes"):
        return True
    config = _load_pomhealer_config(workspace)
    return bool(config.get("pomhealer_mcp", {}).get("auto_after_test", False))
...
def run_pom_propose_all(workspace: Path) -> int:
    ...
    from pomhealer.queue import list_unprocessed_failures
    from pomhealer.pom_propose import process_failure_entry
    from pomhealer.session_state import session_had_healable_failures, session_healable_failure_ids

    if not session_had_healable_failures():
        print("[pomhealer] pom_propose: no session healable failures to process")
        return 0
    ...
    if not pending:
        print("[pomhealer] pom_propose: no pending failures from this session")
        return 0
    ...
    for entry in pending:
        try:
            ...
            print(f"[pomhealer] pom_propose failed for {entry.get('failure_id')}: {exc}")
            rc = 1
    return rc
...
def run_mcp_propose_all(workspace: Path) -> int:
    ...
    from pomhealer.doctor import load_dotenv_files
    from pomhealer.queue import select_latest_awaiting_patches
    from pomhealer.llm_config import resolve_llm_config
    from pomhealer.mcp_propose_runner import process_patch_entry
    from pomhealer.session_state import session_new_patch_ids
    ...
    if not session_patches:
        print(
            "[pomhealer] mcp_propose_runner: no session patches to process "
            "(stale awaiting_agent skipped; "
            "run python -m pomhealer.mcp_propose_runner --patch-id P-... manually)"
        )
        return 0
    ...
    if not session_entries:
        ...
        from pomhealer.queue import list_awaiting_agent, sort_entries_newest_first
    ...
    if not session_entries:
        print(
            "[pomhealer] mcp_propose_runner: session patches are not awaiting_agent "
    ...
    print(
        f"[pomhealer] mcp_propose_runner: processing latest patch "
    ...
    for entry in session_entries:
        ...
        try:
            ...
            print(f"[pomhealer] mcp_propose_runner failed for {patch_id}: {exc}")
            rc = 1
    return rc
...
def should_run_post_test_chain(workspace: Path | None = None) -> bool:
    ...
    if not is_auto_enabled(workspace):
        return False
    from pomhealer.session_state import session_had_healable_failures

    return session_had_healable_failures()


def run_post_test_chain(workspace: Path | None = None) -> int:
    """Run optional post-test healing steps when POMHEALER_MCP_AUTO=1 or config flag set.
    ...
    if not should_run_post_test_chain(workspace):
        print(
            "\n[pomhealer] POMHEALER_MCP_AUTO enabled — skipping post-test chain "
            "(no healable locator failures captured this session)"
        )
        return 0
    print("\n[pomhealer] POMHEALER_MCP_AUTO enabled — running post-test chain (this session only)")
    steps = (
    ...
    for name, runner in steps:
        ...
        if rc != 0:
            failed.append(name)
    if failed:
        print(f"[pomhealer] post-test chain completed with errors: {', '.join(failed)}")
        return 1
    return 0
