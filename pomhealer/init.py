"""Initialize pomhealer layout in a consumer Playwright POM project."""
...
auth_dir = "pomhealer-artifacts/auth"
"""
...
def _templates_root() -> Path:
    try:
        root = resources.files("pomhealer") / "templates"
...
def _write_pomhealer_toml(workspace: Path, *, force: bool = False) -> str | None:
    dest = workspace / "pomhealer-artifacts" / "pomhealer.toml"
    if dest.exists() and not force:
        return None
    dest.parent.mkdir(parents=True, exist_ok=True)
    src = _templates_root() / "pomhealer-artifacts" / "pomhealer.toml"
    if src.exists():
        shutil.copy2(src, dest)
    else:
...
def init_workspace(
    ...
    *,
    ...
    result: dict[str, object] = {
        ...
        "pomhealer_toml": _write_pomhealer_toml(workspace, force=force),
    ...
    from pomhealer.package_sync import refresh_packaged_assets
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Initialize pomhealer artifacts, config, and Cursor skills in a POM project."
    )
    ...
    parser.add_argument(
        "--no-check",
        action="store_true",
        help="Skip pomhealer-doctor after init.",
    )
    parser.add_argument(
        "--verify-mcp",
        action="store_true",
    ...
    if result["pomhealer_toml"]:
        print(f"  wrote {result['healing_toml']}")
    else:
        print("  pomhealer-artifacts/pomhealer.toml already present")
    if result["mcp"]:
        print(f"  wrote {result['mcp']}")
    else:
        print("  .cursor/mcp.json already present")
    if result.get("env_example"):
        print(f"  wrote {result['env_example']} (copy to .env; set POMHEALER_LLM_PROVIDER + matching API key for MCP propose)")
    else:
    ...
    print("  pomhealer-artifacts/ directories ensured")
    ...
    print("  2. cp .env.example .env  # set POMHEALER_LLM_PROVIDER + matching API key for MCP propose")
    print("  3. pytest tests/ -v")
    print("  4. optional: set POMHEALER_MCP_AUTO=1 in .env, then pytest tests/ -v")
    print("  pomhealer-doctor          # re-check setup anytime")
    print("  Full guide: docs/CONSUMER_SETUP.md (in the pomhealer package repo)")
    ...
    if not args.no_check:
        print()
        from pomhealer.doctor import format_report, run_doctor
