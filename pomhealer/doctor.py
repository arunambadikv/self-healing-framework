"""Consumer environment checks (`pomhealer-doctor`)."""
...
def _check_package_import() -> CheckResult:
    try:
        import pomhealer  # noqa: F401

        loc = getattr(pomhealer, "__file__", None) or "(namespace)"
        return CheckResult("pomhealer package", "ok", f"importable ({loc})")
    except Exception as exc:
        return CheckResult("pomhealer package", "error", f"import failed: {exc}")
...
def _check_pytest_plugin() -> CheckResult:
    try:
        ...
        if "pomhealer" in names:
            return CheckResult("pytest plugin", "ok", f"pytest11 entry pomhealer → {names['pomhealer']}")
        return CheckResult(
            "pytest plugin",
            "warn",
            "pytest11 entry 'pomhealer' not found (reinstall package or add -p pomhealer.pytest_plugin)",
        )
    except Exception as exc:
...
def _check_cursor_sdk() -> CheckResult:
    try:
        ...
        return CheckResult(
            "cursor-sdk",
            "warn",
            "not installed — reinstall pomhealer (cursor-sdk ships with the package)",
        )
...
def _check_ci_download_dirs(workspace: Path) -> CheckResult:
    from pomhealer.artifact_import import detect_download_roots, download_dir_is_newer
    ...
    if newer:
        ...
        return CheckResult(
            "CI downloads",
            "warn",
            f"{names} newer than local queue — run pomhealer-import or pomhealer-review to merge",
        )
    if roots:
        ...
        return CheckResult(
            "CI downloads",
            "ok",
            f"leftover {names} (already imported or not newer); pomhealer-import --cleanup to remove",
        )
...
def _check_mcp_json(workspace: Path) -> CheckResult:
    ...
    if not path.exists():
        return CheckResult(
            "mcp.json",
            "warn",
            "missing .cursor/mcp.json — run pomhealer-init (CLI propose still has built-in defaults)",
        )
    try:
...
def _check_api_key(workspace: Path) -> CheckResult:
    from pomhealer.llm_config import KEY_ENV_VARS, LlmConfigError, resolve_llm_config
    ...
    hint = (
        f"set {cfg.key_env} for POMHEALER_LLM_PROVIDER={cfg.provider} "
    ...
    if cfg.provider in ("openai", "gemini", "groq", "litellm"):
        try:
            ...
            return CheckResult(
                "LLM SDK",
                "warn",
                "reinstall pomhealer (openai + mcp ship with the package)",
            )
...
def _check_config(workspace: Path) -> CheckResult:
    candidates = [
        workspace / "pomhealer-artifacts" / "pomhealer.toml",
    ]
    for path in candidates:
        if path.exists():
            return CheckResult("pomhealer.toml", "ok", str(path.relative_to(workspace)))
    pyproject = workspace / "pyproject.toml"
    if pyproject.exists():
        text = pyproject.read_text(encoding="utf-8")
        if "[tool.pomhealer]" in text:
            return CheckResult("pomhealer.toml", "ok", "using [tool.pomhealer] in pyproject.toml")
    return CheckResult(
        "pomhealer.toml",
        "warn",
        "no pomhealer-artifacts/pomhealer.toml — run pomhealer-init (defaults still apply)",
    )
...
def _check_pages_dir(workspace: Path) -> CheckResult:
    ...
    return CheckResult("pages dir", "warn", f"{cfg.pages_dir}/ missing — create page objects or run pomhealer-init")
...
def _check_artifact_dirs(workspace: Path) -> CheckResult:
    from pomhealer.config import load_config
    from pomhealer.paths import configure_workspace, FAILURES_DIR
    ...
    return CheckResult(
        "artifact dirs",
        "warn",
        f"{cfg.artifacts_dir}/ not initialized — run pomhealer-init",
    )


def _check_skills(workspace: Path) -> CheckResult:
    from pomhealer.skill_paths import BUNDLED_SKILLS, resolve_skill_path
...
def format_report(results: list[CheckResult]) -> str:
    ...
    if any(r.name == "LLM API key" and r.status == "warn" for r in results):
        ...
        lines.append(
            "      Automated MCP propose needs POMHEALER_LLM_PROVIDER + matching key "
    ...
    lines.append(
        ...
        "pytest (or pomhealer-doctor) after a package install/update from git."
    )
    return "\n".join(lines)


def main() -> int:
    parser = argparse.ArgumentParser(description="Check consumer setup for the pomhealer package.")
