...
from pomhealer.step_trace import record_step
...
def _is_pomhealer_internal(filename: str) -> bool:
    norm = _norm_path(filename)
    return bool(
        re.search(r"/pomhealer/(playwright_trace|step_trace|pytest_plugin)\.py$", norm)
    )


def _is_base_page_file(filename: str) -> bool:
    norm = _norm_path(filename)
    return norm.endswith("/pomhealer/base_page.py") or norm.endswith("/pages/base_page.py")
...
def attribute_from_frames(frames: list[StackFrameInfo]) -> StepAttribution:
    ...
    for frame in frames:
        if _is_pomhealer_internal(frame.filename):
            continue
...
def _should_auto_record() -> bool:
    ...
    if _in_auto_trace.get():
        return False
    from pomhealer.step_trace import get_active_step_trace
