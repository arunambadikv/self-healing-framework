"""Merge `gh run download` CI artifacts into the local healer-artifacts tree."""
...
from healing.pom_apply import normalize_validation_command, relative_pytest_target

KNOWN_DOWNLOAD_NAMES = ("healing-pipeline-e2e", "healing-artifacts")
...
class ImportResult:
    ...
    def summary_line(self) -> str:
        ...
        return (
            f"[healing] imported {self.failures} failures, {self.patches} patches from {src}"
        )
...
def resolve_artifact_layout(path: Path) -> Path | None:
    """Return the directory that contains failures/ and healing-queue/, if any."""
    if not path.is_dir():
        return None
    if (path / "failures").is_dir() and (path / "healing-queue").is_dir():
        return path.resolve()
    nested = path / "healer-artifacts"
    if (nested / "failures").is_dir() and (nested / "healing-queue").is_dir():
        return nested.resolve()
    return None


def detect_download_roots(workspace: Path) -> list[DownloadRoot]:
    """Find CI download folders that are not the live healer-artifacts tree."""
...
def rewrite_ci_paths(text: str, workspace: Path) -> str:
    ...
    from healing.pom_apply import _CI_PYTHON_RE, _RUNNER_ROOT_RE
...
def import_ci_artifacts(
    ...
    *,
    ...
    """Detect CI download dirs and merge them into healer-artifacts/."""
    from healing.healing_queue import merge_index_entries
    ...
    for root in roots:
        ...
        result.patches += _copy_newer(
            root.layout / "healing-queue" / "patches",
            artifacts / "healing-queue" / "patches",
            dry_run=dry_run,
        )
        for sub in ("applied", "skipped", "summaries"):
            _copy_newer(
                root.layout / "healing-queue" / sub,
                artifacts / "healing-queue" / sub,
                dry_run=dry_run,
            )
        for sub in ("architecture", "healing-reports", "auth"):
            _copy_newer(root.layout / sub, artifacts / sub, dry_run=dry_run)

        if not dry_run:
            result.rewritten += _rewrite_tree(artifacts / "failures", workspace)
            result.rewritten += _rewrite_tree(artifacts / "healing-queue", workspace)

            incoming_index = root.layout / "healing-queue" / "index.json"
...
def download_dir_is_newer(workspace: Path, root: DownloadRoot) -> bool:
    ...
    incoming = root.layout / "healing-queue" / "index.json"
    local = _artifacts_dir(workspace) / "healing-queue" / "index.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Merge gh-run-download CI artifacts into healer-artifacts/."
    )
    ...
    if not result.sources:
        print("[healing] no CI download directories found")
        return 0
    prefix = "[dry-run] " if args.dry_run else ""
    print(f"{prefix}{result.summary_line()}")
    if result.cleaned:
        print(f"[healing] removed download dir(s): {', '.join(result.cleaned)}")
    return 0
