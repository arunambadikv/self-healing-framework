...
from healing.paths import MANIFEST_JSON, MANIFEST_MD, ARCHITECTURE_DIR, ensure_queue_dirs

SCAN_DIRS = ("pages", "tests", "data", "healing")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build architecture manifest for POM healing.")
    ...
    if args.check:
        ...
        old = json.loads(MANIFEST_JSON.read_text(encoding="utf-8"))
        if old.get("content_hash") != manifest.get("content_hash"):
            print("Architecture manifest is stale. Run: python -m healing.architecture_scan")
            return 1
