"""Unit tests for POM pomhealer queue and architecture scan."""

import json
from pathlib import Path

from pomhealer.architecture_scan import build_manifest
...
from pomhealer.paths import configure_workspace, ensure_queue_dirs, FAILURES_DIR, QUEUE_INDEX
...
def test_build_manifest_includes_demo_page(tmp_path: Path, monkeypatch):
    ...
    orangehrm_tests = [
        t for t in manifest.get("tests", []) if str(t.get("file", "")).endswith("test_orangehrm_pomhealer.py")
    ]
...
def test_write_stub_patch_uses_exact_source_expression(tmp_path: Path, monkeypatch):
    from pomhealer.pom_propose import write_stub_patch
    ...
    page_file.write_text(
        '''class DemoPage:
    @property
    def pomhealer_demo_green_button(self) -> Locator:
...
''',
    ...
    failure = {
        ...
        "architecture_ref": "DemoPage.pomhealer_demo_green_button",
...
def test_pomhealer_queue_register_and_propose(tmp_path: Path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    configure_workspace(tmp_path)
    ensure_queue_dirs()
    ...
    patch_json = tmp_path / "pomhealer-artifacts/pomhealer-queue/patches" / f"{patch_id}.json"
...
def test_infer_architecture_from_consumer_traceback():
    from pomhealer.failure_report import build_failure_payload, infer_architecture_from_traceback
...
def test_failure_classifier_auth_failure_not_healable():
    payload = {
        "architecture_ref": "OrangeHrmDashboardPage.pomhealer_demo_dashboard_heading",
        "failing_step": {
            "page_class": "OrangeHrmDashboardPage",
            "method": "expect_pomhealer_demo_dashboard_visible",
            "locator_id": "pomhealer_demo_dashboard_heading",
        },
        "environment": {
...
def test_promote_patch_skill_path_parity(tmp_path: Path, monkeypatch):
    ...
    (pages / "demo_page.py").write_text(
        "class DemoPage:\n"
        "    @property\n"
        "    def pomhealer_demo_green_button(self):\n"
        "        return self.page.get_by_role('button', name='Click Me (Blue)')\n",
        encoding="utf-8",
    )
    ...
    payload = {
        ...
        "error": {"type": "TimeoutError", "message": "locator timeout"},
        "architecture_ref": "DemoPage.pomhealer_demo_green_button",
        "failing_step": {"locator_id": "pomhealer_demo_green_button"},
    }
    json_path = FAILURES_DIR / f"{failure_id}.json"
    md_path = FAILURES_DIR / f"{failure_id}.md"
    json_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        json.dumps(
            {
                "proposal": {
                    ...
                    "architecture_updates": [
                        {
                            "file": "pages/demo_page.py",
                            "symbol": "pomhealer_demo_green_button",
...
def test_ci_gates_skip_queue_gates(tmp_path: Path, monkeypatch):
    from pomhealer.ci_gates import GateConfig, run_ci_gates
    ...
    exit_code, errors, _ = run_ci_gates(
        workspace=tmp_path,
        config=config,
        reports_dir=tmp_path / "pomhealer-artifacts" / "pomhealer-reports",
...
def test_session_state_reset_and_note():
    from pomhealer.session_state import (
...
def test_should_run_post_test_chain_gating(tmp_path: Path, monkeypatch):
    from pomhealer.post_test import should_run_post_test_chain
    from pomhealer.session_state import note_healable_failure, reset_session_state

    reset_session_state()
    monkeypatch.delenv("POMHEALER_MCP_AUTO", raising=False)
    assert not should_run_post_test_chain(tmp_path)

    monkeypatch.setenv("POMHEALER_MCP_AUTO", "1")
    assert not should_run_post_test_chain(tmp_path)
...
def test_pomhealer_mcp_auto_reads_from_dotenv(tmp_path: Path, monkeypatch):
    from pomhealer.post_test import is_auto_enabled
    from pomhealer.paths import configure_workspace, reset_workspace

    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("POMHEALER_MCP_AUTO", raising=False)
    (tmp_path / ".env").write_text("POMHEALER_MCP_AUTO=1\n", encoding="utf-8")
    configure_workspace(tmp_path)
...
def test_auto_architecture_discovery_on_package_stamp_change(tmp_path: Path, monkeypatch):
    from pomhealer.auto_scan import (
    ...
    assert (tmp_path / "pomhealer-artifacts" / "architecture" / "manifest.json").exists()
...
def test_auto_chain_mcp_runs_latest_session_patch(tmp_path: Path, monkeypatch, capsys):
    ...
    from pomhealer.post_test import run_mcp_propose_all
    from pomhealer.session_state import note_session_patch, reset_session_state
    ...
    monkeypatch.setattr("pomhealer.mcp_propose_runner.process_patch_entry", fake_process)
    ...
    monkeypatch.setattr("pomhealer.queue.select_latest_awaiting_patches", fake_select)
    monkeypatch.setattr(
        "pomhealer.queue.list_awaiting_agent",
        lambda: fake_select(),
    )
...
def test_build_agent_prompt_uses_workspace_relative_patch_path(tmp_path: Path, monkeypatch):
    from pomhealer.mcp_propose_runner import build_agent_prompt
    from pomhealer.paths import QUEUE_PATCHES, configure_workspace, ensure_queue_dirs
    ...
    assert "pomhealer-artifacts/pomhealer-queue/patches/P-test123.json" in prompt
...
def test_pom_propose_creates_new_patch_for_duplicate_architecture_ref(tmp_path: Path, monkeypatch):
    ...
    from pomhealer.queue import list_unprocessed_failures, register_failure
    from pomhealer.paths import FAILURES_DIR, configure_workspace, ensure_queue_dirs
    from pomhealer.pom_propose import process_failure_entry
    from pomhealer.session_state import reset_session_state, session_new_patch_ids
...
''',
    ...
    arch_ref = "OrangeHrmLoginPage.pomhealer_demo_login_button"
    ...
    def _write_failure(failure_id: str) -> None:
        payload = {
            ...
            "failing_step": {
                ...
                "method": "click_pomhealer_demo_login",
                "action": "click",
                "locator_id": "pomhealer_demo_login_button",
            },
            "environment": {"base_url": "https://example.com/login"},
            "test": {"file": "tests/test_orangehrm_pomhealer.py", "nodeid": f"tests/t.py::t[{failure_id}]"},
...
def test_list_patch_ready_newest_first(tmp_path: Path, monkeypatch):
    from pomhealer.queue import list_patch_ready, register_failure
    from pomhealer.paths import FAILURES_DIR, QUEUE_INDEX, configure_workspace, ensure_queue_dirs
...
def test_build_failure_payload_includes_storage_state():
    from pomhealer.failure_report import build_failure_payload
    ...
    md = __import__("pomhealer.failure_report", fromlist=["write_failure_markdown"]).write_failure_markdown(
        payload
    )
...
def test_mcp_servers_include_isolated_storage_state(tmp_path: Path, monkeypatch):
    from pomhealer.mcp_propose_runner import resolve_storage_state_path
    from pomhealer.mcp_stdio import load_playwright_mcp_stdio, with_storage_state_args
...
def test_proposal_prompt_prefers_page_url_and_storage_state():
    from pomhealer.pom_propose import build_proposal_prompt

    failure = {
        "failure_id": "F-prompt1",
        "architecture_ref": "OrangeHrmDashboardPage.pomhealer_demo_pim_link",
        ...
        "artifacts": {"storage_state": "pomhealer-artifacts/failures/storage-state-F-prompt1.json"},
        "test_steps": [
...
def test_slugify_test_name_handles_nodeids_and_params():
    from pomhealer.artifact_naming import slugify_test_name
...
def test_new_failure_id_readable_format(tmp_path: Path, monkeypatch):
    from pomhealer.failure_report import new_failure_id
...
def test_new_patch_id_readable_format(tmp_path: Path, monkeypatch):
    from pomhealer.pom_propose import new_patch_id
    from pomhealer.paths import configure_workspace, ensure_queue_dirs
...
def test_artifact_id_dedupes_within_same_second(tmp_path: Path, monkeypatch):
    ...
    from pomhealer.paths import configure_workspace, ensure_queue_dirs, FAILURES_DIR
...
def test_bundled_skills_match_cursor_skills():
    ...
    from pomhealer.skill_paths import BUNDLED_SKILLS

    repo = Path(__file__).resolve().parents[1]
    for name in BUNDLED_SKILLS:
        cursor = repo / ".cursor" / "skills" / name / "SKILL.md"
        tmpl = repo / "pomhealer" / "templates" / "skills" / name / "SKILL.md"
...
def test_load_config_prefers_pomhealer_artifacts_toml(tmp_path: Path, monkeypatch):
    from pomhealer.config import load_config, reset_config_cache
    from pomhealer.paths import configure_workspace, reset_workspace

    monkeypatch.chdir(tmp_path)
    (tmp_path / "pomhealer.toml").write_text('artifacts_dir = "root-artifacts"\n', encoding="utf-8")
    root = tmp_path / "pomhealer-artifacts"
    root.mkdir()
    (root / "pomhealer.toml").write_text(
        'artifacts_dir = "pomhealer-artifacts"\nskills_dir = ".cursor/skills"\n',
        encoding="utf-8",
    )
    configure_workspace(tmp_path)
    reset_config_cache()
    cfg = load_config(tmp_path)
    assert cfg.artifacts_dir == "pomhealer-artifacts"
    reset_workspace()


def test_init_workspace_writes_pomhealer_layout_and_skills(tmp_path: Path, monkeypatch):
    from pomhealer.init import init_workspace
    from pomhealer.paths import FAILURES_DIR, reset_workspace
    ...
    assert result["pomhealer_toml"] == "pomhealer-artifacts/pomhealer.toml"
    assert (tmp_path / "pomhealer-artifacts" / "pomhealer.toml").exists()
    ...
    assert FAILURES_DIR.resolve() == (tmp_path / "pomhealer-artifacts" / "failures").resolve()
    assert (tmp_path / ".cursor" / "skills" / "pomhealer-init" / "SKILL.md").exists()
...
def test_package_sync_refreshes_stale_skills_after_fingerprint_change(tmp_path: Path, monkeypatch):
    from pomhealer.package_sync import (
    ...
    monkeypatch.chdir(tmp_path)
    configure_workspace(tmp_path)
    skill = tmp_path / ".cursor" / "skills" / "pomhealer-review" / "SKILL.md"
    skill.parent.mkdir(parents=True)
    skill.write_text("# stale consumer copy\n", encoding="utf-8")
    (tmp_path / "pomhealer-artifacts" / "architecture").mkdir(parents=True)
...
def test_pin_playwright_mcp_config_updates_latest_pin(tmp_path: Path, monkeypatch):
    ...
    from pomhealer.package_sync import pin_playwright_mcp_config
    from pomhealer.paths import configure_workspace, reset_workspace
...
def test_doctor_reports_ok_for_package_and_warns_without_api_key(tmp_path: Path, monkeypatch):
    from pomhealer.doctor import run_doctor
    from pomhealer.init import init_workspace
    from pomhealer.paths import reset_workspace
...
def test_find_chromium_executable_scans_cache(tmp_path: Path, monkeypatch):
    from pomhealer.doctor import find_chromium_executable
...
def test_load_pomhealer_yaml_falls_back_to_package(tmp_path: Path):
    from pomhealer.gates_config import load_pomhealer_yaml

    # No workspace pomhealer/ci_gates_config.yaml
    data = load_pomhealer_yaml(tmp_path)
    assert "thresholds" in data or "pomhealer_mcp" in data or "test_policy" in data
...
def test_list_deferred_and_queue_lock_roundtrip(tmp_path: Path, monkeypatch):
    from pomhealer.queue import list_deferred, update_patch_status
...
def test_validation_command_allowlist():
    from pomhealer.pom_apply import is_validation_command_allowed, parse_validation_command

    assert is_validation_command_allowed(["pytest", "tests/t.py", "-q"])
    assert is_validation_command_allowed(["python", "-m", "pytest", "tests/t.py"])
    assert is_validation_command_allowed(["python", "-m", "pomhealer.ci_gates"])
...
def test_apply_patch_rolls_back_on_validation_failure(tmp_path: Path, monkeypatch):
    from pomhealer.pom_apply import apply_patch_payload
...
def test_mcp_propose_runner_with_mocked_agent(tmp_path: Path, monkeypatch):
    ...
    from pomhealer.mcp_propose_runner import process_patch_entry
    from pomhealer.paths import QUEUE_PATCHES
    ...
    monkeypatch.setenv("POMHEALER_LLM_PROVIDER", "cursor")
...
def test_resolve_llm_config_defaults_and_keys(monkeypatch):
    from pomhealer.llm_config import LlmConfigError, resolve_llm_config
    ...
    monkeypatch.delenv("GOOGLE_API_KEY", raising=False)
    monkeypatch.delenv("POMHEALER_LLM_MODEL", raising=False)
    monkeypatch.delenv("POMHEALER_MCP_MODEL", raising=False)
    ...
    monkeypatch.setenv("POMHEALER_LLM_PROVIDER", "openai")
    ...
    monkeypatch.setenv("POMHEALER_LLM_PROVIDER", "gemini")
    ...
    monkeypatch.setenv("POMHEALER_LLM_PROVIDER", "groq")
    ...
    monkeypatch.setenv("POMHEALER_LLM_PROVIDER", "litellm")
    ...
    monkeypatch.setenv("POMHEALER_LLM_PROVIDER", "lite_llm")
    cfg = resolve_llm_config()
    assert cfg.provider == "litellm"

    monkeypatch.setenv("POMHEALER_LLM_PROVIDER", "nope")
    try:
        resolve_llm_config()
...
def test_openai_provider_mocked_tool_loop(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "pomhealer.propose_providers.get_provider",
...
def test_openai_tool_call_payload_preserves_gemini_thought_signature():
    ...
    from pomhealer.propose_providers.mcp_tool_loop import openai_tool_call_payload
...
def test_retry_delay_seconds_parses_gemini_message():
    from pomhealer.propose_providers.mcp_tool_loop import retry_delay_seconds
...
def test_invalid_tool_feedback_parses_groq_browser_open_file():
    from pomhealer.propose_providers.mcp_tool_loop import invalid_tool_feedback
...
def test_mcp_tools_to_openai_keeps_pomhealer_subset_only():
    from types import SimpleNamespace

    from pomhealer.propose_providers.mcp_tool_loop import mcp_tools_to_openai
...
def test_is_capacity_or_size_error_detects_groq_413():
    from pomhealer.propose_providers.mcp_tool_loop import is_capacity_or_size_error
...
def test_workspace_file_tools_allow_patches_and_reject_pages(tmp_path: Path):
    from pomhealer.propose_providers.mcp_tool_loop import run_workspace_file_tool

    patch = tmp_path / "pomhealer-artifacts/pomhealer-queue/patches/P-demo.json"
    ...
    wrote = run_workspace_file_tool(
        ...
        {"path": "pomhealer-artifacts/pomhealer-queue/patches/P-demo.json", "content": '{"ok": true}'},
    )
...
def test_ci_workflow_mentions_multi_provider_secrets():
    ...
    ci = (root / ".github/workflows/pomhealer-ci.yml").read_text(encoding="utf-8")
    assert "POMHEALER_LLM_PROVIDER" in ci
    ...
    e2e = (root / ".github/workflows/pomhealer-pipeline-e2e.yml").read_text(encoding="utf-8")
    assert "POMHEALER_LLM_PROVIDER" in e2e
    assert "LITE_LLM_KEY" in e2e
    assert "steps.llm.outputs.has_key" in e2e
    assert "pomhealer-review --interactive" in e2e


def test_normalize_validation_command_rewrites_ci_python(tmp_path: Path):
    from pomhealer.pom_apply import normalize_validation_command, parse_validation_command
    import sys
    ...
    assert "-q" in argv
    assert "--run-pomhealer-demo" in argv


def test_choose_merged_entry_keeps_local_applied():
    from pomhealer.queue import choose_merged_entry
...
def test_move_patch_file_archives_agent_task_and_leaves_pending_empty(tmp_path: Path, monkeypatch):
    ...
    from pomhealer.paths import QUEUE_APPLIED, QUEUE_PATCHES, reset_workspace
...
def test_archive_terminal_patch_files_does_not_overwrite_existing_applied(tmp_path: Path, monkeypatch):
    from pomhealer.queue import _save_index, archive_terminal_patch_files
    from pomhealer.paths import QUEUE_APPLIED, QUEUE_PATCHES, reset_workspace
...
def test_artifact_import_merges_download_and_rewrites_paths(tmp_path: Path, monkeypatch):
    ...
    from pomhealer.queue import _load_index
    from pomhealer.paths import reset_workspace
    ...
    src = tmp_path / "pomhealer-pipeline-e2e"
    (src / "failures").mkdir(parents=True)
    (src / "pomhealer-queue" / "patches").mkdir(parents=True)
    ...
    (src / "failures" / f"{failure_id}.md").write_text(
        f"Screenshot: {runner}/pomhealer-artifacts/failures/shot.png\n",
        encoding="utf-8",
    )
    patch = {
        "proposal": {
            ...
            "links": {
                "failure_json": f"{runner}/pomhealer-artifacts/failures/{failure_id}.json",
            },
            "architecture_updates": [
    ...
    (src / "pomhealer-queue" / "patches" / f"{patch_id}.json").write_text(
        json.dumps(patch, indent=2), encoding="utf-8"
    )
    (src / "pomhealer-queue" / "index.json").write_text(
        json.dumps(
            {
                "version": 1,
                "entries": [
                    {
                        ...
                        "status": "patch_ready",
                        "failure_json": f"{runner}/pomhealer-artifacts/failures/{failure_id}.json",
                        "failure_md": f"{runner}/pomhealer-artifacts/failures/{failure_id}.md",
                        "patch_json": f"{runner}/pomhealer-artifacts/pomhealer-queue/patches/{patch_id}.json",
                        "patch_md": None,
    ...
    assert "pomhealer-pipeline-e2e" in result.sources
    dest_failure = json.loads(
        (tmp_path / "pomhealer-artifacts" / "failures" / f"{failure_id}.json").read_text(encoding="utf-8")
    )
    assert dest_failure["test"]["file"] == "tests/test_demo.py"
    dest_patch = json.loads(
        (tmp_path / "pomhealer-artifacts" / "pomhealer-queue" / "patches" / f"{patch_id}.json").read_text(
            encoding="utf-8"
        )
    )
    cmd = dest_patch["proposal"]["validation_command"]
    from pomhealer.pom_apply import parse_validation_command
    import sys
...
def test_artifact_import_keeps_local_applied_over_incoming_ready(tmp_path: Path, monkeypatch):
    from pomhealer.artifact_import import import_ci_artifacts
    from pomhealer.queue import _load_index, _save_index
    from pomhealer.paths import QUEUE_APPLIED, QUEUE_PATCHES, reset_workspace
    ...
    src = tmp_path / "pomhealer-pipeline-e2e"
    nested = src / "pomhealer-artifacts"
    (nested / "failures").mkdir(parents=True)
    (nested / "pomhealer-queue" / "patches").mkdir(parents=True)
    (nested / "failures" / f"{failure_id}.json").write_text("{}", encoding="utf-8")
    (nested / "pomhealer-queue" / "patches" / f"{patch_id}.json").write_text(
        '{"proposal": {"patch_id": "P-keep-applied"}}', encoding="utf-8"
    )
    (nested / "pomhealer-queue" / "patches" / f"{patch_id}-agent-task.md").write_text(
        "# leftover task", encoding="utf-8"
    )
    (nested / "pomhealer-queue" / "index.json").write_text(
        json.dumps(
            {
                "version": 1,
                "entries": [
    ...
    result = import_ci_artifacts(tmp_path)
    assert result.sources == ["pomhealer-pipeline-e2e"]
    index = _load_index()
    entry = next(e for e in index["entries"] if e.get("patch_id") == patch_id)
...
def test_doctor_warns_on_sandbox_browsers_path(monkeypatch):
    from pomhealer.doctor import _check_playwright_browsers
...
def test_pomhealer_import_console_script_is_packaged():
    root = Path(__file__).resolve().parents[1]
    text = (root / "pyproject.toml").read_text(encoding="utf-8")
    assert 'pomhealer-import = "pomhealer.artifact_import:main"' in text
